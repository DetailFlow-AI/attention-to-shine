...
// TASK 1 — BOOKING TIME BLOCKS — DONE 2026-07-01
// Each booking reserves the 5 hours starting at the booked time — the detail
// plus buffer that prevents double bookings. A 10:00 AM booking blocks
// 10:00 AM – 3:00 PM; nothing before the start time is blocked. The
// availability check uses the same window: a slot is taken if the 5-hour block
// starting there would overlap an existing event, so a slot stays open only
// when a full 5-hour window fits before the next booking.
