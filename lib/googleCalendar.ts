...
// Each booking reserves a 5-hour window starting at the booked time — the
// detail itself plus a buffer for travel/setup/overrun, to prevent double
// bookings. A 10:00 AM booking blocks 10:00 AM – 3:00 PM; nothing before the
// start time is blocked. The availability check uses the same window: a slot
// is taken if the 5-hour window starting there would overlap an existing
// event, so a slot stays open only when a full window fits before the next
// booking.
// TASK 1 (booking time blocks) — DONE 2026-06-16: block is 5h per spec
// (10AM → 10AM–3PM). Keep BookingForm.tsx DETAIL_HOURS in sync with this.
...
export async function createBookingEvent(ev: BookingEvent): Promise<CalendarResult> {
  ...
  try {
    const token = await getAccessToken(saEmail, saKey);
    // Reserve the 5-hour window starting at the booked time (detail + buffer).
