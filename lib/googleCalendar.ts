...
// plus a buffer that prevents double-booking. A 10:00 AM booking blocks
// 10:00 AM – 3:00 PM; nothing before the start time is blocked. The
// availability check uses the same window: a slot is taken if the 5-hour
// reservation starting there would overlap an existing event, so a slot stays
// open only when a full 5-hour window fits before the next booking.
// TASK 1 (time blocking) — DONE 2026-06-25: block 5 hours forward per booking.
...
export async function createBookingEvent(ev: BookingEvent): Promise<CalendarResult> {
  ...
  try {
    const token = await getAccessToken(saEmail, saKey);
    // Reserve the 5 hours starting at the booked time (the detail plus
    // buffer). A 10:00 AM booking blocks 10:00 AM – 3:00 PM on the calendar.
