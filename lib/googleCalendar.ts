...
// Each booking reserves the 5 hours starting at the booked time — a buffer
// that prevents double bookings. A 10:00 AM booking blocks 10:00 AM – 3:00 PM;
...
export async function createBookingEvent(ev: BookingEvent): Promise<CalendarResult> {
  ...
  try {
    const token = await getAccessToken(saEmail, saKey);
    // Reserve the 5 hours starting at the booked time (buffer against double
