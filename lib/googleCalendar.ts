...
// nothing before the start time is blocked. This 5-hour buffer prevents
// double bookings. The availability check uses the same window: a slot is
// taken if the 5-hour detail starting there would overlap an existing event,
// so a slot stays open only when a full 5-hour window fits before the next
// booking.
