...
// Each booking reserves the 5 hours starting at the booked time — the working
// window for a detail plus buffer, sized to prevent double bookings. A 10:00 AM
...
// block starting there would overlap an existing event, so a slot stays open
// only when a full block fits before the next booking.
// TASK 1 (5-hour booking time blocks) — DONE 2026-06-21
...
export async function createBookingEvent(ev: BookingEvent): Promise<CalendarResult> {
  ...
  try {
    ...
    // overlapping booking can be scheduled in that window.
