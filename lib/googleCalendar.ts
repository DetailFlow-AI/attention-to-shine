...
// Each booking reserves a 5-hour buffer starting at the booked time — the
// length of a detail plus setup/travel margin — to prevent double bookings.
// A 10:00 AM booking blocks 10:00 AM – 3:00 PM; nothing before the start time
// is blocked. The availability check uses the same window: a slot is taken if
// the 5-hour block starting there would overlap an existing event, so a slot
// stays open only when a full 5-hour window fits before the next booking.
// TASK 1 (5-hour time blocking) — DONE 2026-07-01
...
export async function createBookingEvent(ev: BookingEvent): Promise<CalendarResult> {
  ...
  try {
    const token = await getAccessToken(saEmail, saKey);
    // Reserve a 5-hour buffer starting at the booked time to prevent double
    // bookings. A 10:00 AM booking blocks 10:00 AM – 3:00 PM on the calendar.
