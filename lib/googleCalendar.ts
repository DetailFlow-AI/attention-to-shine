...
// TASK 1 — BOOKING TIME BLOCKS — DONE 2026-06-26
// Each booking reserves the 5 hours starting at the booked time — a buffer that
// prevents double bookings within the window a detail (plus travel/setup) can
// occupy. A 10:00 AM booking blocks 10:00 AM – 3:00 PM; nothing before the
// start time is blocked. The availability check uses the same window: a slot is
// taken if the 5-hour block starting there would overlap an existing event, so
// a slot stays open only when a full 5-hour window fits before the next booking.
...
export async function createBookingEvent(ev: BookingEvent): Promise<CalendarResult> {
  ...
  try {
    const token = await getAccessToken(saEmail, saKey);
    // Reserve the 5 hours starting at the booked time (the buffer window).
