...
// TASK 1 — BOOKING TIME BLOCKS — DONE 2026-06-16
// Each booking reserves the 5 hours starting at the booked time — this is the
// buffer that prevents double bookings. A 10:00 AM booking blocks
...
export async function createBookingEvent(ev: BookingEvent): Promise<CalendarResult> {
  ...
  try {
    const token = await getAccessToken(saEmail, saKey);
    // Reserve the 5 hours starting at the booked time (prevents double bookings).
