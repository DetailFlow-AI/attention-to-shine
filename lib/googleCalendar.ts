...
// Each booking reserves a 5-hour window starting at the booked time — enough
// to cover the detail plus travel/buffer and prevent double bookings. A
...
// open only when a full window fits before the next booking.
...
export async function createBookingEvent(ev: BookingEvent): Promise<CalendarResult> {
  ...
  try {
    const token = await getAccessToken(saEmail, saKey);
    // Reserve the 5-hour window starting at the booked time.
