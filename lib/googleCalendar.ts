...
// Each booking reserves a 5-hour window starting at the booked time — this is
// a double-booking buffer that covers the detail plus travel/setup/overrun. A
...
//
// Task 1 (5-hour booking blocks) — DONE 2026-06-15.
...
export async function createBookingEvent(ev: BookingEvent): Promise<CalendarResult> {
  ...
  try {
    const token = await getAccessToken(saEmail, saKey);
    // Reserve the 5-hour window starting at the booked time (double-booking
