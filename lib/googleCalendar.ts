...
// Each booking reserves a 5-hour buffer starting at the booked time — enough
// to cover the detail itself plus travel/cleanup, and to prevent double
...
// is taken if the 5-hour buffer starting there would overlap an existing
// event, so a slot stays open only when a full 5-hour window fits before the
// next booking.
// TASK 1 (time blocking) DONE 2026-06-17 — 5-hour block per booking.
...
export async function createBookingEvent(ev: BookingEvent): Promise<CalendarResult> {
  ...
  try {
    const token = await getAccessToken(saEmail, saKey);
    // Reserve the 5-hour buffer starting at the booked time.
