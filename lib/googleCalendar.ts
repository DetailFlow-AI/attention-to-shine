...
export interface BookingEvent {
  ...
  durationHours?: number; // length of the booking block in hours from the start (default 5)
}
...
// TASK 1 — TIME BLOCKING — DONE 2026-06-25
// Each booking reserves the 5 hours starting at the booked time — the detail
// itself plus a buffer that prevents back-to-back double bookings. A 10:00 AM
// booking blocks 10:00 AM – 3:00 PM; nothing before the start time is blocked.
// The availability check uses the same window: a slot is taken if the 5-hour
// window starting there would overlap an existing event, so a slot stays open
// only when a full 5-hour window fits before the next booking.
...
export async function createBookingEvent(ev: BookingEvent): Promise<CalendarResult> {
  ...
  try {
    ...
    // A 10:00 AM booking blocks 10:00 AM – 3:00 PM on the calendar so no new
    // booking can be scheduled during that window.
