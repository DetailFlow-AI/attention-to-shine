...
// of a detail plus buffer, preventing double bookings. A 10:00 AM booking
...
export async function createBookingEvent(ev: BookingEvent): Promise<CalendarResult> {
  ...
  try {
    const token = await getAccessToken(saEmail, saKey);
    // Reserve the 5 hours starting at the booked time (detail + buffer) to
    // prevent double bookings. A 10:00 AM booking blocks 10:00 AM – 3:00 PM.
