...
/**
 * The gallery grid and its lightbox. Slots arrive already resolved from the
 * filesystem by the page (a server component); empty ones render as
 * placeholders and aren't clickable.
 */
export default function GalleryGrid({ slots }: { slots: GallerySlot[] }) {
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null);

  // Only slots with a photo can appear in the lightbox, so its indices come
  // from this filtered list rather than from the grid.
  const filledSlots = slots.filter((s) => s.src);
