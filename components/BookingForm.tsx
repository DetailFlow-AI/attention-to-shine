...
export default function BookingForm({ defaultService }: { defaultService?: string }) {
  ...
  // Each booking reserves a 5-hour block starting at its time (a 10:00 AM
  // booking blocks 10:00 AM–3:00 PM) — nothing before it — to prevent double
  // bookings. So a slot is bookable only if a full 5-hour window starting there
  // fits before the next booking, i.e. its [X, X+5) window doesn't overlap any
  ...
  // would start at `slot`, or null if a block fits. Drives both the disabled
