...
export default function BookingForm({ defaultService }: { defaultService?: string }) {
  ...
  // Each booking reserves a 5-hour window starting at its time (a 10:00 AM
  // booking blocks 10:00 AM–3:00 PM) — nothing before it. This is a
  // double-booking buffer covering the detail plus travel/setup/overrun. So a
  // slot is bookable only if a full 5-hour window starting there fits before
  // the next booking, i.e. its [X, X+5) window doesn't overlap any existing
  // booking. Mirrors BOOKING_BLOCK_HOURS in lib/googleCalendar.ts.
  ...
  // The soonest existing booking whose window overlaps the 5-hour window that
  // would start at `slot`, or null if it fits. Drives both the disabled slots
  // and the "detail at …" explanation.
