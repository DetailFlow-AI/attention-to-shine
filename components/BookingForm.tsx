...
export default function BookingForm({ defaultService }: { defaultService?: string }) {
  ...
  // Each booking reserves 5 hours starting at its time (a 10:00 AM booking
  // blocks 10:00 AM–3:00 PM) — the detail plus a buffer that prevents double
  // bookings — nothing before it. So a slot is bookable only if a full 5-hour
  // block starting there fits before the next booking, i.e. its [X, X+5) window
  // doesn't overlap any existing detail. Mirrors BOOKING_BLOCK_HOURS in
  // lib/googleCalendar.ts. (TASK 1 — DONE 2026-06-25)
