...
export default function BookingForm({ defaultService }: { defaultService?: string }) {
  ...
  // booking blocks 10:00 AM–3:00 PM) — the detail plus a buffer, nothing before
  // it. So a slot is bookable only if a full 5-hour window starting there fits
  // before the next booking, i.e. its [X, X+5) window doesn't overlap any
  // existing booking. Mirrors BOOKING_BLOCK_HOURS in lib/googleCalendar.ts.
  // TASK 1 (booking time blocks) — DONE 2026-06-16: 5h window per spec.
