...
export default function BookingForm({ defaultService }: { defaultService?: string }) {
  ...
  // blocks 10:00 AM–3:00 PM) — detail plus buffer, nothing before it. So a slot
  // is bookable only if a full 5-hour window starting there fits before the
  // next booking, i.e. its [X, X+5) window doesn't overlap any existing
  // booking. Mirrors the calendar block (BOOKING_BLOCK_HOURS) in
